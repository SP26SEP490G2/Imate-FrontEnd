                        ...
                        <div className="bg-[#11142D] border border-white/10 rounded-2xl p-6 shadow-2xl backdrop-blur-sm sticky top-6">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-lg font-bold flex items-center gap-2">
                                    <Filter size={18} className="text-purple-400" /> Bộ lọc
                                </h2>
                                <Button
                                    variant="ghost"
                                    size="sm"
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Status filter (custom buttons instead of select for better UI) */}
                                <div className="space-y-3">
                                    <label className="text-xs font-medium text-slate-400 uppercase tracking-wider">Trạng thái hồ sơ</label>
                                    <div className="space-y-2">
                                        {allStatuses.map(status => (
                                            <button
                                                key={status.value}
                                                onClick={() => setStatusFilter(status.value as ApplicationStatus | "All")}
                                                className={`w-full text-left px-4 py-2.5 rounded-xl text-sm transition-all flex items-center justify-between ${statusFilter === status.value
                                                    ...
                                                    : "bg-[#0F1333] text-slate-300 border border-white/5 hover:border-white/10"
                                                    }`}
                                            >
                                    ...
                                    {data.items.map((job) => {
                                        ...
                                        return (
                                            <div key={job.id} className="bg-[#11142D] border border-white/10 rounded-2xl p-6 hover:border-purple-500/40 transition-all duration-300 shadow-xl relative overflow-hidden group">
                                                {/* Card Glow Effect */}
                                                <div className="absolute top-0 right-0 w-32 h-32 bg-purple-600/5 blur-2xl group-hover:bg-purple-600/10 transition-all pointer-events-none" />
