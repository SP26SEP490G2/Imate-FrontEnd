...
const InterviewSchedule = () => {
  ...
  const fetchBookings = async () => {
    ...
    setError(null);
    try {
      let data: BookingDetailResponse[] = [];
      if (user.role === "Candidate") {
        data = await getCandidateBookings();
      } else if (user.role === "Mentor") {
        data = await getMentorBookings();
      }
      setBookings(data || []);
    } catch (err: any) {
      console.error("Error fetching bookings:", err);
      // fallback mock message or error
      setError("Không thể tải dữ liệu lịch hẹn.");
      // toast.error("Có lỗi xảy ra khi tải lịch hẹn. Vui lòng thử lại!"); // MSG-07
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="text-white p-6 max-w-6xl mx-auto h-[calc(100vh-80px)] overflow-y-auto">
      {/* HEADER */}
      <h1 className="text-3xl font-bold mb-8">Lịch phỏng vấn</h1>
        ...
        {isLoading ? (
          <div className="flex justify-center items-center py-20">
           ...
           <div className="flex flex-col justify-center items-center py-20 text-gray-400">
            ...
            <p className="text-lg">Không thể tải dữ liệu lịch hẹn! (MSG-07)</p>
          ...
          <div className="flex flex-col justify-center items-center py-20 text-gray-400">
            ...
            <p className="text-lg">Bạn không có lịch hẹn nào vào ngày này. (MSG-06)</p>
          </div>
        ) : (
