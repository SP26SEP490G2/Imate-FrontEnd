...
function PaginationLink({ className, isActive, size = "icon", ...props }: PaginationLinkProps) {
  return (
    <a
      ...
      className={cn(buttonVariants({ size }), isActive ? "border-primary-500 border bg-[#5D5FEF] text-white hover:bg-indigo-700" : "border border-neutral-300 bg-white text-neutral-500 hover:border-gray-400 hover:bg-gray-100 hover:text-gray-700", "rounded-md px-3 py-1 transition-colors", className)}
      {...props}
    />
  );
}
