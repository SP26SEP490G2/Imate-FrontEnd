...
function Header() {
  ...
  const messageRefs = useRef<Record<number, HTMLParagraphElement | null>>({});
  ...
  useEffect(() => {
    if (!isOpenNotificationMenu) return;

    setOverflowMessageIds((prev) => {
      const next = new Set(prev);

      notifications.forEach((notification) => {
        const el = messageRefs.current[notification.id];
        if (!el) return;
  ...
  }, [notifications, isOpenNotificationMenu, showAllNotifications, expandedNotificationId]);
                        ...
                        displayedNotifications.map((notification) => (
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <p
                                ref={(el) => {
                                  messageRefs.current[notification.id] = el;
                                }}
                                className={cn(
                              ...
                              {(expandedNotificationId === notification.id || overflowMessageIds.has(notification.id)) && (
                                <button
                                  type="button"
